...
# 接触力在 batch 里的字段名；TA-VLA 默认训练和推理都要求提供这个字段。
OBS_EFFORT = "observation.effort"

from ..pretrained import PreTrainedPolicy, T
from ..rtc.modeling_rtc import RTCProcessor
from .configuration_ta import DEFAULT_IMAGE_SIZE, TAVLAConfig
...
class PaliGemmaWithExpertModel(
    ...
    """PaliGemma model with action expert for TA-VLA."""
...
class TAVLAPytorch(nn.Module):  # see openpi `PI0Pytorch`
    """Core TA-VLA PyTorch model."""

    def __init__(self, config: TAVLAConfig, rtc_processor: RTCProcessor | None = None):
        super().__init__()
        self.config = config
        ...
        # 这就是“把历史力信息加进模型”的入口。
        effort_dim_in = config.effort_dim * len(config.effort_history)
        self.effort_proj_in = nn.Linear(effort_dim_in, 2 * action_expert_config.width)
        self.effort_proj_out = nn.Linear(2 * action_expert_config.width, action_expert_config.width)
    ...
    def gradient_checkpointing_enable(self):
        ...
        logging.info("Enabled gradient checkpointing for TAVLAPytorch model")
    ...
    def gradient_checkpointing_disable(self):
        ...
        logging.info("Disabled gradient checkpointing for TAVLAPytorch model")
    ...
    def _project_effort(self, effort_history: Tensor) -> Tensor:
        # effort_history: [B, history_len, effort_dim]
        # 展平成 [B, history_len * effort_dim]，让 MLP 同时看到历史窗口里的全部力信息。
        bsize = effort_history.shape[0]
        effort_flat = effort_history.reshape(bsize, -1)
        effort_hidden = self.effort_proj_in(effort_flat)
        effort_hidden = F.silu(effort_hidden)
        # 返回 [B, 1, width]，也就是一个“力 token”，后面会拼到 suffix token 序列里。
    ...
    def embed_suffix(self, noisy_actions, timestep, effort_history: Tensor | None = None):
        ...
        # timestep 是 flow matching 的时间条件，仍然沿用 PI0.5 的正弦位置编码。
        ...
        # suffix 内部的 attention 结构：
        # 力 token 和第一个动作 token 是新的块，后续动作 token 按自回归/因果方式接在后面。
        # 结果是：动作 token 能利用历史力，但 prefix 的图像/语言 token 不会反向看动作 token。
    ...
    def forward(self, images, img_masks, tokens, masks, actions, noise, time, effort_history: Tensor | None = None) -> Tensor:
        ...
        suffix_embs, suffix_pad_masks, suffix_att_masks, adarms_cond = self.embed_suffix(x_t, time, effort_history)
        ...
        # suffix 里第一个 token 是历史力 token；真正要预测速度场的是最后 chunk_size 个未来 token。
    ...
    def sample_actions(
        ...
        tokens,
        masks,
        effort_history: Tensor | None = None,
        noise=None,
        num_steps=None,
    ...
    def denoise_step(
        ...
        suffix_embs, suffix_pad_masks, suffix_att_masks, adarms_cond = self.embed_suffix(x_t, timestep, effort_history)
...
class TAVLAPolicy(PreTrainedPolicy):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        print(
            "The TA-VLA model is adapted from the PI0.5/OpenPI implementation with effort conditioning. \n"
        ...
        try:
            ...
            for key, current_value in current_state_dict.items():
                if key not in remapped_state_dict:
                    remapped_state_dict[key] = current_value
                    continue

                loaded_value = remapped_state_dict[key]
                if loaded_value.shape != current_value.shape:
                    resized_value = current_value.clone()
                    common_slices = tuple(slice(0, min(src, dst)) for src, dst in zip(loaded_value.shape, current_value.shape, strict=True))
                    resized_value[common_slices] = loaded_value[common_slices].to(
                        device=current_value.device, dtype=current_value.dtype
                    )
                    remapped_state_dict[key] = resized_value
                    print(f"Resized checkpoint tensor for {key}: {tuple(loaded_value.shape)} -> {tuple(current_value.shape)}")
    ...
    def _fix_pytorch_state_dict_keys(
        ...
        for key, value in state_dict.items():
            ...
            # TA-VLA/pi05-compatible model expects time_mlp_*, but checkpoint might have action_time_mlp_*
            if key.startswith("action_time_mlp_in."):
                new_key = key.replace("action_time_mlp_in.", "time_mlp_in.")
            elif key.startswith("action_time_mlp_out."):
                new_key = key.replace("action_time_mlp_out.", "time_mlp_out.")
            # Also handle state_proj which should not exist in TA-VLA
            if key.startswith("state_proj."):
                logging.warning(f"Skipping state_proj key in TA-VLA mode: {key}")
                continue
    ...
    def prepare_effort(self, batch, *, require_future: bool) -> tuple[Tensor, Tensor | None]:
        ...
        if effort is None:
            raise ValueError(f"{OBS_EFFORT} is required for TA-VLA")

        effort = effort.to(device=device, dtype=torch.float32)
        if effort.ndim == 2:
            # 允许只有当前一帧力的输入 [B, effort_dim]，这里补成 [B, 1, effort_dim]。
            effort = effort[:, None, :]

        if effort.shape[-1] != self.config.effort_dim:
            raise ValueError(f"Expected {OBS_EFFORT} last dim {self.config.effort_dim}, got {effort.shape[-1]}")
        if effort.shape[1] < history_len:
            raise ValueError(f"Expected at least {history_len} effort history steps, got {effort.shape[1]}")
    ...
    def forward(self, batch: dict[str, Tensor], reduction: str = "mean") -> tuple[Tensor, dict]:
        ...
        losses = self.model.forward(images, img_masks, tokens, masks, actions, noise, time, effort_history)
    ...
    def _get_default_peft_targets(self) -> dict[str, any]:
        """Return default PEFT target modules for TA-VLA fine-tuning."""
        # LoRA/PEFT 微调时也把新加的力投影层纳入可训练目标。
        common_projections = (
            "action_in_proj|action_out_proj|time_mlp_in|time_mlp_out|effort_proj_in|effort_proj_out"
        )
