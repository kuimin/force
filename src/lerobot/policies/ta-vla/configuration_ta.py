...
# 数据集中接触力字段的名字。训练和推理都会从 batch["observation.effort"] 取力信息。
...
@PreTrainedConfig.register_subclass("ta-vla")
@dataclass
class TAVLAConfig(PreTrainedConfig):
    ...
    # 状态和动作会按原 PI0.5 逻辑 pad 到固定维度；接触力不 pad，而是单独按 effort_dim 校验。
